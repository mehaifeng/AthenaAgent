...
- **Parallel Tool Calls**: `ToolCallParallelism` batches a turn's tool calls (`Security.MaxParallelToolCalls`, default 4). Only *consecutive* read-only calls are batched — never reordered across a write — and only under Off/Balanced approval, where read-only calls provably never prompt. Results are always written back in the original order to satisfy the `tool_calls` pairing constraint. Set 1 to restore strictly sequential execution.
