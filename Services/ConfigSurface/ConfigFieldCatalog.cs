...
public static class ConfigFieldCatalog
{
    ...
    private static IReadOnlyList<ConfigFieldDescriptor> Build()
    {
        ...
        [
            ...
            Field("Security.MaxParallelToolCalls", "Security", ConfigFieldType.Integer,
                c => c.MaxParallelToolCalls, (c, v) => c.MaxParallelToolCalls = (int)v!,
                "How many tool calls from one assistant turn may run at once. Only runs of consecutive read-only calls are batched; writes, deletions and terminal commands stay strictly sequential, as does everything under Strict or Automatic approval. Set 1 to execute every call one at a time.",
                range: (1, 16)),
            Field("Security.MaxToolResultChars", "Security", ConfigFieldType.Integer,
                c => c.MaxToolResultChars, (c, v) => c.MaxToolResultChars = (int)v!,
                "Max chars any single tool result may add to the conversation context. Oversized results are compressed at the registry chokepoint (long array tails dropped first, then long text water-filled) and flagged with a truncationNote.",
                range: (2_000, 2_000_000)),

            // —— Extensions ——
            Field("Extensions.OfficeToolsMode", "Extensions", ConfigFieldType.Enum,
                c => c.OfficeToolsMode.ToString(),
                (c, v) => c.OfficeToolsMode = Enum.Parse<OfficeToolsMode>((string)v!, ignoreCase: true),
                "How the native OOXML toolset is exposed. Auto shows only enable_office_tools until the model unlocks it (saves ~35% of the tool declarations in conversations that never touch Office files); Always exposes all 15 tools; Off hides them entirely.",
                allowedValues: EnumNames<OfficeToolsMode>()),
