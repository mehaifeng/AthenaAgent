...
- **⏰ cron 定时任务**：标准五段 cron 调度。每次触发都会开出一个全新会话，而不是往你正在看的对话里插消息，因此任务可以在你继续工作的同时跑。
...
- `Services/`：核心业务逻辑，包括 AI 集成、文件系统管理和 cron 定时任务（`Services/Cron/`）。
...
- **Markdown**：LiveMarkdown.Avalonia（流式输出、内联图片、数学公式与原生 Mermaid 图表）
