...
public static class AnalyzeCommand
{
    ...
    public static Command Create()
    {
        ...
        cmd.SetAction((parseResult) =>
        {
            ...
            var outlineLevels = stylesDoc?.Descendants(W + "style")
                .Select(style => new
                {
                    Id = (string?)style.Attribute(W + "styleId"),
                    Level = (string?)style.Element(W + "pPr")?.Element(W + "outlineLvl")?.Attribute(W + "val")
                })
                .Where(item => item.Id != null && int.TryParse(item.Level, out var level) && level is >= 0 and <= 8)
                .ToDictionary(item => item.Id!, item => int.Parse(item.Level!))
            ...
            foreach (var p in body.Descendants(W + "p"))
            {
                ...
                var isNamedHeading = style?.StartsWith("Heading", StringComparison.OrdinalIgnoreCase) == true;
                if (style != null && (isNamedHeading || outlineLevels.TryGetValue(style, out _)))
                {
                    var text = string.Concat(p.Descendants(W + "t").Select(t => t.Value));
                    int? level = outlineLevels.TryGetValue(style, out var outlineLevel)
                        ? outlineLevel + 1
                        : isNamedHeading && style.Length > 7 && int.TryParse(style.AsSpan(7), out var namedLevel)
                            ? namedLevel
                            : null;
                    headings.Add(new { style, level, text });
                }
            }
            ...
            // Styles
            var styleNames = stylesDoc == null
                ? []
                : stylesDoc.Descendants(W + "style")
                    .Where(s => IsTrue((string?)s.Attribute(W + "customStyle")))
