...
def find_soffice() -> str | None:
    ...
    Search order:
      1. Platform default install locations
...
def recalculate(
    ...
    with tempfile.TemporaryDirectory(prefix="xlsx_recalc_") as tmpdir:
        ...
        tmp_input = os.path.join(input_dir, os.path.basename(input_path))
        ...
        tmp_output = os.path.join(output_dir, stem + ".xlsx")

        if not os.path.isfile(tmp_output):
            # Try to find any .xlsx file in tmpdir (LibreOffice may behave differently)
            xlsx_files = [f for f in os.listdir(output_dir) if f.lower().endswith(".xlsx")]
            if xlsx_files:
                tmp_output = os.path.join(output_dir, xlsx_files[0])
            else:
                stdout = result.stdout.decode(errors="replace").strip()
                return False, (
                    f"LibreOffice succeeded (exit 0) but output file not found in {output_dir}.\n"
                    f"stdout: {stdout}\n"
                    f"Files in output directory: {os.listdir(output_dir)}"
                )

        # Copy recalculated file to final destination
        final_path = os.path.abspath(output_path)
        os.makedirs(os.path.dirname(final_path), exist_ok=True)
        staged_path = final_path + f".{os.getpid()}.tmp"
        try:
            shutil.copy(tmp_output, staged_path)
