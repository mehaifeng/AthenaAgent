...
最终收尾审计已关闭（2026-07-28）：

- 删除无调用方的旧 History 单会话替换路径及视图事件桥，只保留会话树创建独立 VM 的恢复 seam。
- 修复 Onboarding、Logs 和 Knowledge Base 对长生命周期发布者的订阅释放，并补充幂等释放测试。
- `ConfigService` 在配置文件尚不存在时也缓存默认配置，首次启动不再产生多个内存实例。
- 删除旧单 Provider、根级模型凭据、角色级无效采样参数以及无调用方的凭据解析模块。
- 删除旧 Tab/History localization、过时注释与历史评审中的旧类型路径。
